<template>
  <div>
    <content-page loadingMessage="Loading Giveaways..." :loading="loadingGiveaways" :noContent="!giveaways?.length">
      <template v-slot:contextual-content>
        <v-card class="cyberpunk-border frosted">
          <v-card-title class="text-h5"> <v-icon class="mr-2">mdi-wallet</v-icon>Credits </v-card-title>
          <v-card-text class="d-flex">
            ...
            <v-card class="pa-4 frosted" v-if="hasDeployedDCLScene">
              <v-card-text class="flex-grow-1">
                ...
                <div class="text-display mb-4" v-if="userUnclaimedGdaCredits">
                  You have deployed a <strong>Decentraland</strong> scene, which makes you eligible for complimentary giveaway credits!
                </div>
                <div class="text-display mb-4" v-else>
                  You have already claimed your 10,000 complimentary credits. <br />
                  Thanks for participating in the Great Decentraland Airdrop!
                </div>
                <div class="text-display mb-4" v-if="userUnclaimedGdaCredits">
                  You have <strong>{{ userUnclaimedGdaCredits }}</strong> Promotional Credits left to claim.
                </div>
              </v-card-text>
    ...
    <v-dialog v-model="airdropClaimDialog" width="500">
      <v-card>
        <v-card-title class="text-h5"> Decentraland Airdrop Claim </v-card-title>
        ...
        <v-card-text class="d-flex flex-column justify-center">
          ...
          <div class="text-display text-left">
            Thanks to a grant from the Decentraland DAO, your deployed Decentraland scene allows you to claim up to 10,000 complimentary Airdrop
            Credits.
          </div>
          <div class="text-display text-right" v-if="userUnclaimedGdaCredits">
            <strong>Promotional Credits Remaining:</strong>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="claimAirdropCredits"> Claim </v-btn>
          <v-btn color="grey" text @click="airdropClaimDialog = false"> Cancel </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="allocationDialog" width="500">
      <v-card>
        <v-card-title class="text-h5"> Allocate Credits To Giveaway </v-card-title>
        ...
        <v-card-text>
          ...
          <v-autocomplete
            v-if="giveawayAllocationList.length"
            label="Giveaways"
            hide-details
            outlined
            :items="giveawayAllocationList"
            v-model="allocationGiveawayId"
          />
        </v-card-text>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="allocateCreditBalance"> Allocate </v-btn>
          <v-btn color="grey" text @click="allocationDialog = false"> Cancel </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
  ...
  computed: {
    ...
    pastGiveaways() {
      return this.giveaways
        .filter((giveaway) => giveaway.endTime && DateTime.fromMillis(giveaway?.endTime).diffNow() < 0)
    ...
    ongoingGiveaways() {
      return [...this.giveaways]
        ...
        .filter(
          (giveaway) =>
            DateTime.fromMillis(giveaway.startTime).diffNow() < 0 && (!giveaway.endTime || DateTime.fromMillis(giveaway?.endTime).diffNow() > 0)
        )
    },
    ...
    futureGiveaways() {
      return this.giveaways.filter((giveaway) => DateTime.fromMillis(giveaway.startTime).diffNow() > 0)
    },
  },
  methods: {
    ...mapActions({
