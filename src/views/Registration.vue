  ...
  <v-card elevation="2" class="mx-auto pb-4" max-width="960">
    ...
    <v-card-text>
      <v-card elevation="12" class="pa-6 my-2">
        <v-card-title class="d-flex justify-space-between"> User Profile </v-card-title>
        <v-card-subtitle>
          Looks like this is your first time connecting this wallet. Let's set up your account. (You'll be able to make changes later.)
        </v-card-subtitle>
        <v-card-text>
          <v-text-field label="Primary Wallet Address" v-model="newUserInfo.connectedWallet" disabled></v-text-field>
          <v-text-field label="Display Name" v-model="newUserInfo.displayName" :rules="[validateDisplayName]"></v-text-field>
      ...
      <v-card elevation="12" class="pa-6 my-2">
        ...
        <v-card-subtitle> Select additional features you'd like to use. </v-card-subtitle>
        <v-card-text>
          <div class="d-flex">
            <v-switch
              label="Advanced User"
          ...
          <div class="d-flex">
            <v-switch label="Organization Admin" hint="Enables team administration features" persistent-hint v-model="newUserRoles[4]"></v-switch>
          </div>
        </v-card-text>
      </v-card>
      <v-card elevation="12" class="pa-6" v-if="orgAdmin">
        <v-card-title> Create Organization </v-card-title>
        <v-card-subtitle> Set a name for your organization - you'll be able to make more changes later. </v-card-subtitle>
        <v-card-text>
          <v-text-field label="Organization Display Name" v-model="newOrg.displayName" class="mr-2" :rules="[validateOrgDisplayName]"></v-text-field>
        </v-card-text>
      </v-card>
    </v-card-text>
