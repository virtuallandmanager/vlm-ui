            ...
            <v-col>
              <div class="d-flex flex-column justify-space-between">
                ...
                <v-text-field label="Email Address"
                  hint="Optional - For future notifications and multi-factor authentication" persistent-hint
                  validate-on-blur :rules="[validateEmailAddress]" v-model="userInfo.emailAddress"
                  class="mr-2"></v-text-field>
                <vue-tel-input-vuetify class="ml-2 flex-grow-1"
                  :preferred-countries="['us', 'ca', 'ar', 'gb', 'fr', 'de']" v-model="phone" label="Mobile Phone"
                  hint="Optional - For future notifications and multi-factor authentication"
                  :rules="[validatePhoneNumber]" disabledFetchingCountry persistent-hint
                  :defaultCountry="userInfo.location?.country_code || 'us'" @input="onInput" />
              </div>
              ...
              <div class="d-flex">
                <v-switch label="Advanced User"
                  ...
                  v-model="userRoles[2]"></v-switch>
              </div>
              <div class="text-h6">Demo Features</div>
              <v-switch label="Hide Demo Scene" v-model="userInfo.hideDemoScene" persistent-hint
                hint="Removes the Public Demo Scene from your list of scenes"></v-switch>
            </v-col>
          </v-row>
        </v-container>
      </v-card-text>
  ...
  methods: {
    ...
    validatePhoneNumber() {
      if (!this.userInfo?.smsPhoneNumber) {
        ...
        this.userInfo?.smsPhoneNumber?.number &&
        this.userInfo?.smsPhoneNumber?.valid
      ) {
        return true;
      } else {
    ...
    validateEmailAddress() {
      if (
        ...
        !this.userInfo.emailAddress.includes("@")
      ) {
