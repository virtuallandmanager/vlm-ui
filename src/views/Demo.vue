<template>
  <focus-page :loadingMessage="`Connecting To Demo Scene...`" :loading="processing || loadingPreset" :noContent="!scene" :imageLink="scene?.imageLink || placeholder">
    <template v-slot:header>{{ scene?.name }}</template>
    <template v-slot:header-actions>
      <div class="d-flex align-center flex-center">
        <v-fade-transition>
          <v-chip v-if="connected" :color="connectedColor" :border-color="borderColor">Connected To Demo Scene </v-chip>
        </v-fade-transition>
      </div>
    </template>
    ...
    <v-card class="cyberpunk-border pa-6">
