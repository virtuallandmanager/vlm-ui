<template>
  <div>
    <focus-page :loadingMessage="`Loading ${event?.name}`" :loading="loadingEvent" :noContent="!event"
      :imageLink="placeholder">
      <template v-slot:header>{{ event?.name }}</template>
      <template v-slot:header-actions><v-chip v-if="ongoing" class="green ma-4"><span>Ongoing</span></v-chip>
        <v-chip v-else-if="future" class="orange ma-4"><span>Starts {{ startTimeRelative }}</span></v-chip>
            ...
            <v-row class="d-flex justify-space-between">
              <v-col md="4" sm="6" xs="6">
                <div class="d-flex flex-column align-stretch justify-stretch flex-grow-2">
                  <v-text-field label="Event Name" v-model="event.name" outlined :disabled="propertyLoading('name')"
                    :loading="propertyLoading('name')" @change="updateEventProperty('name')"></v-text-field>
                  <v-textarea label="Event Description" v-model="event.description" outlined
                    :disabled="propertyLoading('description')" :loading="propertyLoading('description')"
                    @change="updateEventProperty('description')" :rules="[event?.description?.length <= 500]"
              ...
              <v-col md="8" sm="6" xs="6">
                <v-row>
                  <v-col>
                    <v-card class="date-time-card frosted cyberpunk-border flex-grow-0" elevation="12">
                      <v-card-title v-if="future"
                        class="date-time-title justify-space-around text-center">Starts</v-card-title>
                      <v-card-title v-if="ongoing"
                        class="date-time-title justify-space-around text-center">Started</v-card-title>
                      <v-card-title v-if="!event.eventStart"
                        class="date-time-title justify-space-around text-center">Event
                        Start</v-card-title>
                      <v-card-text class="justify-space-around text-center flex-column flex-grow-1">
                        ...
                        <div v-if="event.eventStart" class="date-text justify-space-around text-center">{{
                          startDateTimeLocal }}</div>
                      </v-card-text>
                      <v-card-actions class="d-flex flex-column justify-center pa-2">
                        <v-menu v-model="editingStartDate" :close-on-content-click="false" max-width="290">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn text color="primary" x-small v-bind="attrs" v-on="on"
                              :loading="propertyLoading('eventStartDate')"><v-icon class="mr-1"
                                x-small>mdi-calendar</v-icon>Set
                              Date</v-btn>
                          </template>
                          <v-card>
                            <v-date-picker color="primary" v-model="eventStartDate"
                              @change="updateEventProperty('eventStartDate')"></v-date-picker>
                        ...
                        <v-menu v-model="editingStartTime" :close-on-content-click="false" max-width="290">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn text color="primary" v-bind="attrs" v-on="on" x-small
                              :loading="propertyLoading('eventStartTime')"><v-icon class="mr-1"
                                x-small>mdi-clock</v-icon>Set
                              Time</v-btn>
                          </template>
                          <v-card>
                            <v-time-picker v-if="editingStartTime" color="primary" v-model="eventStartTime"
                              @change="updateEventProperty('eventStartTime')"></v-time-picker>
                            <v-card-actions class="d-flex justify-end">
                        ...
                        <v-btn v-if="event.eventStart" text color="primary" x-small
                          :loading="propertyLoading('eventStart')" @click="clearStartDate"><v-icon class="mr-1"
                            x-small>mdi-close</v-icon>Clear</v-btn>
                      </v-card-actions>
                    </v-card>
                  </v-col>
                  <v-col>
                    <v-card dense class="date-time-card frosted cyberpunk-border" elevation="12">
                      <v-card-title v-if="eventEndDate && !past"
                        class="date-time-title justify-space-around text-center">Ends</v-card-title>
                      <v-card-title v-if="eventEndDate && past"
                        class="date-time-title justify-space-around text-center">Ended</v-card-title>
                      <v-card-title v-if="!event.eventEnd" class="date-time-title justify-space-around text-center">Event
                        End</v-card-title>
                      <v-card-text class="">
                        <div v-if="!eventEndDate && !eventEndTime" class="date-text text-center" :min-height="42"><v-icon
                            x-large>mdi-infinity</v-icon></div>
                        <div v-if="eventEndDate && eventEndTime" class="date-text justify-space-around text-center">{{
                          endTimeRelative
                        }}
                        </div>
                        <div v-if="event.eventEnd" class="date-text justify-space-around text-center">{{ endDateTimeLocal
                        }}
                        </div>
                      </v-card-text>
                      <v-card-actions class="d-flex flex-column justify-center pa-2">
                        <v-menu v-model="editingEndDate" :close-on-content-click="false" max-width="290">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn text color="primary" x-small v-bind="attrs" v-on="on"
                              :loading="propertyLoading('eventEndDate')"><v-icon class="mr-1"
                                x-small>mdi-calendar</v-icon>Set
                              Date</v-btn>
                          </template>
                          <v-card>
                            <v-date-picker color="primary" v-model="eventEndDate"
                              @change="updateEventProperty('eventEndDate')"></v-date-picker>
                            <v-card-actions class="d-flex justify-end">
                        ...
                        <v-menu v-model="editingEndTime" :close-on-content-click="false" max-width="290">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn text color="primary" v-bind="attrs" v-on="on" x-small
                              :loading="propertyLoading('eventEndTime')"><v-icon class="mr-1"
                                x-small>mdi-clock</v-icon>Set
                              Time</v-btn>
                          </template>
                          <v-card>
                            <v-time-picker v-if="editingEndTime" color="primary" v-model="eventEndTime"
                              @change="updateEventProperty('eventEndTime')"></v-time-picker>
                            <v-card-actions class="d-flex justify-end">
                        ...
                        <v-btn v-if="event.eventEnd" text color="primary" x-small :loading="propertyLoading('eventEnd')"
                          @click="clearEndDate"><v-icon class="mr-1" x-small>mdi-close</v-icon>Clear</v-btn>
                      </v-card-actions>
                    </v-card>
                  </v-col>
                </v-row>
                <v-row>
                  <v-col>
                    <v-select label="Linked Scenes" multiple outlined :items="userSceneSelection" item-text="name"
                      item-value="sk" v-model="sceneLinkIds" :loading="propertyLoading('linkedScenes')"
                      @change="sceneLinksUpdate"></v-select>
                    <v-select label="Linked Giveaways" multiple outlined :items="userGiveawaySelection" item-text="name"
                      item-value="sk" v-model="giveawayLinkIds" :loading="propertyLoading('linkedGiveaways')"
                      @change="giveawayLinksUpdate"></v-select>
                  </v-col>
                  <v-col>
                    <v-card class="black pa-2">
                      <claim-limits :itemName="event.name" :claimLimits="event.claimLimits"
                        @updateClaimLimits="updateEventProperty('claimLimits')"></claim-limits>
                    </v-card>
                  </v-col>
                </v-row>
              </v-col>
    ...
    <v-dialog v-model="linkSceneDialog" max-width="300">
      <v-card>
        ...
        <v-card-text>
          <v-select outlined :items="userSceneList" label="Scene" item-text="name" item-value="sk" v-model="sceneToLink"
    ...
    <v-dialog v-model="linkGiveawayDialog" max-width="300">
      <v-card>
        ...
        <v-card-text>
          <v-select outlined :items="userGiveawayList" label="Giveaway" item-text="name" item-value="sk"
            v-model="giveawayToLink" hide-details></v-select>
        </v-card-text>
        <v-card-actions class="d-flex justify-end">
...
export default {
  ...
  computed: {
    ...
    eventEndTime: {
      ...
      set(value) {
        if (value && this.event.eventEnd) { // Check if the value and this.eventEnd are not null or undefined
          const parts = value.split(':');
