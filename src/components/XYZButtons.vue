  ...
  <v-container>
    ...
    <v-row>
      <v-col>
        <v-btn
          v-if="value == 'scale'"
          :outlined="!maintainProportions"
          :color="maintainProportions ? 'primary' : 'grey'"
          class="mt-2"
          max-width="250"
          @click="maintainProportions = !maintainProportions"
          ><v-icon small class="mr-2">{{ maintainProportions ? 'mdi-lock' : 'mdi-lock-open-variant' }}</v-icon> Maintain Proportions</v-btn
        >

        <v-btn-toggle v-if="value == 'position'" exclusive>
          <v-btn :outlined="!moveDiagonalNESW" :color="moveDiagonalNESW ? 'primary' : 'grey'" max-width="250" @click="selectDiagonalNESW"
            ><v-icon small>mdi-arrow-top-right-bottom-left</v-icon></v-btn
          >
          <v-btn
            v-if="value == 'position'"
            :outlined="!moveDiagonalNWSE"
            :color="moveDiagonalNWSE ? 'primary' : 'grey'"
            max-width="250"
            @click="selectDiagonalNWSE"
            ><v-icon small>mdi-arrow-top-left-bottom-right</v-icon></v-btn
          >
        </v-btn-toggle>
      </v-col>
    </v-row>
