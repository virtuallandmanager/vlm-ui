  ...
  <v-card elevation="6">
    ...
    <div class="black d-flex justify-end align-center pa-2">
      <div class="flex-grow-1">
        <v-slider color="white" v-model="video.volume" @change="updateVolume" prepend-icon="mdi-volume-high" max="1" min="0" step=".1" class="ma-0" dense hide-details="auto"></v-slider>
        <v-slider color="white" v-model="video.emission" @change="updateEmission" prepend-icon="mdi-television-ambient-light" max="1" min="0" step=".1" class="ma-0" dense hide-details="auto"></v-slider>
      </div>
      ...
      <v-btn icon dark @click.stop="openClickEventDialog()">
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
    ...
    <div class="grey darken-2 d-flex justify-space-between align-center">
      ...
      <v-switch v-model="video.enableLiveStream" :disabled="!video.liveLink" label="Enable Live Streaming" color="red" hide-details @change="updateLiveToggle" class="ma-0 pr-4"></v-switch>
    </div>
    <div class="my-0 pa-4 d-flex">
      <v-text-field outlined v-model="video.liveLink" label="Live Video Link" hide-details="auto" :rules="[validateStreamLink]" :persistent-hint="true" @blur="updateLiveLink" hint="Enter a .m3u8 link to enable live streaming"> </v-text-field>
    </div>
    ...
    <div v-if="video.offType == 2">
      ...
      <div v-for="(item, v) in video.playlist" :key="v" class="d-flex align-center pa-4">
        <v-text-field outlined v-model="video.playlist[v]" :label="`Video ${v + 1}`" placeholder="Enter Video URL" hide-details="true" @blur="updatePlaylist()" class="flex-grow-1" />
        <v-btn icon @click.stop="openPlaylistItemDeleteDialog(v)" v-if="video.playlist.length > 1">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-icon v-bind="attrs" v-on="on"> mdi-trash-can </v-icon>
            </template>
...
export default {
  ...
  methods: {
    ...
    removePlaylistItem() {
      const i = this.video.playlist.findIndex((video) => video == this.selectedPlaylistItem);
