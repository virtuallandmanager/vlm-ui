  ...
  <div>
    <v-dialog v-model="mintingRightsDialog" max-width="390">
      <v-card>
        ...
        <v-card-text>
          <div class="text-body-1">This giveaway item requires minting rights to be granted to the giveaway contract
            address.</div>
          <div class="text-body-1">Please grant minting rights to the giveaway address and then refresh this
            page.</div>
        </v-card-text>
        <v-card-actions>
        ...
        <v-row>
          <v-col>
            <div class="d-flex flex-column justify-center align-center">
              <v-img :src="item.imageSrc" class="item-image mb-1" :class="bgColor" :width="size || 100"
                :height="size || 100" contain> </v-img>
              <v-chip class="giveaway-item-data text-truncate mb-1" x-small>{{ item.rarity.capitalize() }} {{
                item.category.capitalize() }}</v-chip>
            </div>
          </v-col>
      ...
      <v-container>
        <v-row>
          ...
          <v-col class="d-flex justify-center align-end flex-grow-1">
            <v-btn v-if="item.minterRights" disabled x-small class="mt-2">Minting Rights Not Granted</v-btn>
            <v-btn v-else x-small class="mt-2" @click="showMintingRightsDialog">Must Grant Minting Rights</v-btn>
          </v-col>
        </v-row>
      </v-container>
    </v-card>
