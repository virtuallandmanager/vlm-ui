    ...
    <v-card elevation="6">
      <div class="d-flex justify-space-between align-center primary pa-4">
        ...
        <div class="text-h5 flex-grow-1" v-if="editingName">
          <v-text-field autofocus outlined color="white" label="Model Name" v-model="model.name" hide-details="auto"
            append-outer-icon="mdi-content-save" @click:append-outer="toggleEditMode()" @blur="toggleEditMode()" dense
            @change="editModelName()"></v-text-field>
        </div>
      </div>
      ...
      <div class="px-2 pt-2">
        <v-text-field outlined dark label="Model File" v-model="model.modelSrc" :error-messages="errors"
          @input="validateInput" @change="updateModelSrc()" persistent-hint
          hint="Model files must exist in your project."></v-text-field>
      </div>
      <div>
